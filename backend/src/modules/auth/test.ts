...
} from "../../shared/testing/enterprise-test-kit";
...
interface MutableAuthMiddleware {
  tokenService: {
...
interface MutableAuthService {
  captchaService: {
    verify: ReturnType<typeof vi.fn>;
    start: ReturnType<typeof vi.fn>;
    extractCaptchaValue: ReturnType<typeof vi.fn>;
  };
  mfaService: {
    issueChallenge: ReturnType<typeof vi.fn>;
    issueReplacementChallenge: ReturnType<typeof vi.fn>;
    getChallenge: ReturnType<typeof vi.fn>;
    saveChallenge: ReturnType<typeof vi.fn>;
    verifyCode: ReturnType<typeof vi.fn>;
    clearChallenge: ReturnType<typeof vi.fn>;
  };
  totpService: {
    createSecret: ReturnType<typeof vi.fn>;
    buildOtpAuthUri: ReturnType<typeof vi.fn>;
    verify: ReturnType<typeof vi.fn>;
  };
  secService: {
  ...
  deviceTrustService: {
    validateTrustedSession: ReturnType<typeof vi.fn>;
    createTrustedSession: ReturnType<typeof vi.fn>;
  };
}
...
function createAuthService() {
  ...
  const service = new AuthService(
    ...
    roleRepository as unknown as ConstructorParameters<typeof AuthService>[1],
    passService as unknown as PassService,
    tokenService as unknown as ConstructorParameters<typeof AuthService>[3],
    emailService as any,
    smsService as any,
  );
  const mutable = service as unknown as MutableAuthService;
  mutable.captchaService = {
    start: vi.fn().mockResolvedValue({ captchaCode: "ABCD12" }),
    verify: vi.fn().mockResolvedValue(true),
    extractCaptchaValue: vi.fn(
      (input: { captchaCode?: string; captchaToken?: string }) =>
        input.captchaCode ?? input.captchaToken,
    ),
  };
  mutable.mfaService = {
...
describe("auth module enterprise test suite", () => {
  ...
  describe("domain rules", () => {
    it("normalizes email value object deterministically", () => {
      expect(Email.create("  OWNER@KrishiPath.COM ").toString()).toBe(
        "owner@krishipath.com",
      );
    });
  ...
  describe("token utilities and password security", () => {
    ...
    it("hashes and verifies password without storing plaintext", async () => {
      ...
      await expect(
        passService.verify("StrongPass#123", result.hash, result.salt),
      ).resolves.toBe(true);
      await expect(
        passService.verify("WrongPass#123", result.hash, result.salt),
      ).resolves.toBe(false);
    });
  });
  ...
  describe("auth service business flows", () => {
    ...
    it("rotates refresh tokens and persists new refresh hash", async () => {
      ...
      authRepository.findUserById.mockResolvedValue(
        createUser({ companyId: "company-1" }),
      );
  ...
  describe("middleware security", () => {
    it("rejects missing bearer token", async () => {
      ...
      await AuthMiddleware.ensureAuthenticated(
    ...
    it("rejects inactive session after valid token", async () => {
      const mutable = AuthMiddleware as unknown as MutableAuthMiddleware;
      ...
      await AuthMiddleware.ensureAuthenticated(
        createAuthenticatedRequest("Bearer token"),
        response as Response,
        next as NextFunction,
      );
    ...
    it("attaches auth context for valid active session", async () => {
      ...
      const requestObject = createAuthenticatedRequest(
        "Bearer token",
      ) as Request & {
        auth?: unknown;
      };
    ...
    it("denies restricted subscription at full-access gate", () => {
      ...
      AuthMiddleware.ensureFullAccess(
        requestObject,
        response as Response,
        next as NextFunction,
      );
  ...
  describe("controller and API integration", () => {
    it("returns module status through route integration", async () => {
      ...
      app.use(
        "/auth",
        new AuthRoutes(
          new AuthController(service as unknown as AuthService),
        ).getRouter(),
      );
  ...
  describe("concurrency, idempotency, and event readiness", () => {
    it("creates one session per concurrent successful login finalization", async () => {
      ...
      await Promise.all([
        ...
        service.login(
          { ...validLogin, deviceId: "device-2" },
          { ipAddress: "10.0.0.2", userAgent: "B" },
        ),
      ]);
