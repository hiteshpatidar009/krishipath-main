...
export class MandiPublicService {
  constructor(
    ...
    private readonly productExtrasRepo: ProductExtrasRepository,
    private readonly traderPriceRepo: TraderPriceRepository,
    private readonly marketInsightRepo: MarketInsightRepository,
  ) {}

  private shapeMandi(mandi: any, origin?: Coordinates) {
  ...
  private shapePriceHistory(rows: any[]) {
    return rows.map((row) => ({
      date: row.priceDate,
  ...
  private shapeProduct(crop: any, historyRows: any[]) {
  ...
  private shapeInsight(insight: any) {
    if (!insight) {
  ...
  private async getBuyers(mandiId: string, productId: string) {
    const rows = await this.traderPriceRepo.findByMandiAndProduct(mandiId, productId);
    const byTrader = new Map<string, any>();
  ...
  public async getOverview(
    ...
    if (!mandi || mandi.status !== "ACTIVE") throw new AppError("Mandi not found", 404);
    ...
    const crops = requested.size
      ...
      : enabledCrops;
    const historyByCrop = new Map<string, any[]>();
  ...
  public async getProductDetail(mandiId: string, productId: string, origin?: Coordinates) {
    ...
    const variantsByClassification = new Map<string, any[]>();
  ...
  public async compareProduct(
    ...
    const latestByMandi = new Map<string, any>();
