...
import {
  ...
  mandisTable,
  productsTable,
  farmerCalendarEventsTable,
  farmerTasksTable,
  farmerMarketWatchlistTable,
...
export class FarmerRepository {
      ...
      const [shortStats, followerStats] = await Promise.all([
        this.db
          .select({
            ...
            views: sql<number>`coalesce(sum(${contentShortsTable.views}), 0)::int`,
            likes: sql<number>`coalesce(sum(${contentShortsTable.likes}), 0)::int`,
          })
  ...
  private async findFarmerId(userId: string) {
  ...
  public async updateNotificationPreferences(userId: string, notificationPreferences: Record<string, boolean>) {
  ...
  public async saveMarketWatch(userId: string, mandiId: string, productId: string) {
    ...
    const [existing] = await this.db
      ...
      .from(farmerMarketWatchlistTable)
      .where(and(
        eq(farmerMarketWatchlistTable.farmerId, farmerId),
        eq(farmerMarketWatchlistTable.mandiId, mandiId),
        eq(farmerMarketWatchlistTable.productId, productId),
      ))
      .limit(1);
  ...
  public async getCalendarEvents(userId: string, fromDate: string, toDate: string) {
    const farmerId = await this.findFarmerId(userId);
    if (!farmerId) return [];
    return this.db
      .select({
      ...
      .where(and(
        eq(farmerCalendarEventsTable.farmerId, farmerId),
        gte(farmerCalendarEventsTable.eventDate, fromDate as any),
        lte(farmerCalendarEventsTable.eventDate, toDate as any),
      ))
  ...
  public async upsertProfile(userId: string, data: Partial<typeof farmersTable.$inferInsert>) {
    ...
    if (existing) {
      ...
      await this.db.insert(farmersTable).values({
        ...
        ...data,
      } as typeof farmersTable.$inferInsert);
      return this.findByUserId(userId);
    }
  }

  public async setPreferredMandis(farmerId: string, mandiIds: string[]) {
    // Delete existing
    await this.db
    ...
    if (mandiIds.length > 0) {
      await this.db.insert(farmerMandisTable).values(
        mandiIds.map((id, index) => ({
  ...
  public async setTrackedCrops(farmerId: string, productIds: string[]) {
    // Delete existing
    await this.db
    ...
    if (productIds.length > 0) {
      await this.db.insert(farmerCropsTable).values(
