...
export class RewardRepository {
  ...
  private async findFarmerId(userId: string, db: any = this.db): Promise<string | null> {
  ...
  public async getWalletBalance(farmerId: string): Promise<number> {
  ...
  public async getSummaryByUserId(userId: string) {
    const farmerId = await this.findFarmerId(userId);
    if (!farmerId) return { balance: 0, lifetimeEarned: 0, redeemed: 0, history: [] };
    ...
    const [walletRows, history, redeemedRows] = await Promise.all([
      ...
      this.db.select().from(krishiPointsLedgerTable)
        ...
        .limit(50),
      this.db.select({
        total: sql<number>`coalesce(sum(case when ${krishiPointsLedgerTable.points} < 0 then -${krishiPointsLedgerTable.points} else 0 end), 0)`,
  ...
  public async getCatalogByUserId(userId: string) {
    const farmerId = await this.findFarmerId(userId);
  ...
  public async createCatalogItem(data: typeof rewardCatalogTable.$inferInsert) {
    const [created] = await this.db.insert(rewardCatalogTable).values(data).returning();
    return created;
  }

  public async updateCatalogItem(id: string, data: Partial<typeof rewardCatalogTable.$inferInsert>) {
  ...
  public async redeem(userId: string, catalogItemId: string) {
    return this.db.transaction(async (tx) => {
      ...
      if (!farmerId) throw new AppError("Farmer profile is required", 409, "FARMER_PROFILE_REQUIRED");
      ...
      const [item] = await tx
        ...
        .limit(1);
      if (!item) throw new AppError("Reward is not available", 404, "REWARD_NOT_AVAILABLE");
      ...
      if (item.stock != null) {
        ...
        if (!stockUpdated) throw new AppError("Reward is out of stock", 409, "REWARD_OUT_OF_STOCK");
      }
      ...
      const [wallet] = await tx
        ...
        .where(and(
          eq(farmerWalletsTable.farmerId, farmerId),
          gte(farmerWalletsTable.balance, item.pointsCost),
        ))
        .returning();
      if (!wallet) throw new AppError("Not enough KrishiPoints", 409, "INSUFFICIENT_KRISHIPOINTS");
  ...
  public async addPoints(farmerId: string, actionId: string, points: number, description?: string): Promise<void> {
