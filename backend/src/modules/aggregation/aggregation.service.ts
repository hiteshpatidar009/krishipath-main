...
export class AggregationEngineService {
   ...
   */
  public async recalculateMandiReferencePrice(mandiId: string, productId: string, variantId: string) {
    ...
    const submissions = await db.select().from(priceSubmissionsTable).where(and(
      eq(priceSubmissionsTable.mandiId, mandiId),
      eq(priceSubmissionsTable.productId, productId),
      eq(priceSubmissionsTable.variantId, variantId),
      eq(priceSubmissionsTable.status, "VERIFIED"),
      gt(priceSubmissionsTable.recordedAt, today)
    ));
    ...
    const [existingRef] = await db.select().from(mandiReferencePricesTable).where(and(
      eq(mandiReferencePricesTable.mandiId, mandiId),
      eq(mandiReferencePricesTable.productId, productId),
      eq(mandiReferencePricesTable.variantId, variantId)
    )).limit(1);
