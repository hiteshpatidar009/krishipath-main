...
export class KrishiGuruRepository {
  ...
  public async resolveFarmerId(userId: string, claimedFarmerId?: string): Promise<string | null> {
    if (claimedFarmerId) {
  ...
  public async getTodayUsage(farmerId: string): Promise<number> {
  ...
  public async incrementUsage(farmerId: string): Promise<void> {
  ...
  public async getChatHistory(farmerId: string, limit = 50) {
  ...
  public async saveMessage(farmerId: string, role: string, content: string) {
    // Find or create chat
