...
import {
  marketSourcesTable,
  marketSourceProductsTable,
  marketSourceMandisTable,
  marketSourceParserProfilesTable,
  marketSourceTimelineTable,
  marketSourceAnalyticsTable,
} from "../../infrastructure/database/postgres/schemas/db1/market-source.schema";
import { productsTable, mandisTable } from "../../infrastructure/database/postgres/schemas/db1";
import axios from "axios";

export class MarketSourceService {
  public async getMarketSources(filters: any) {
  ...
  public async getMarketSourceById(id: string) {
    const db = Db1Connection.getInstance();
  ...
  public async createMarketSource(data: any, userId?: string) {
    const db = Db1Connection.getInstance();
    ...
    if (sourceData.stateId === "") sourceData.stateId = undefined;
    if (sourceData.districtId === "") sourceData.districtId = undefined;
    if (sourceData.mandiId === "") sourceData.mandiId = undefined;
  ...
  private async notifyWhatsAppIntelligence(source: any) {
    try {
  ...
  public async updateMarketSource(id: string, data: any, userId?: string) {
    const db = Db1Connection.getInstance();
  ...
  public async getWhatsAppMessages(id: string, limit: number = 20) {
  ...
  public async submitPrices(marketSourceId: string, prices: any[], userId?: string) {
    const db = Db1Connection.getInstance();
    ...
    for (const price of prices) {
      ...
      if (!actualVariantId) {
         const existingVariants = await db.select()
            ...
            .where(and(
                eq(productVariantsTable.productId, price.productId),
                eq(productVariantsTable.gradeName, vName)
            )).limit(1);
  ...
  public async getPriceHistory(marketSourceId: string) {
    const db = Db1Connection.getInstance();
    const { priceSubmissionsTable } = await import("../../infrastructure/database/postgres/schemas/db1/price.schema");
  ...
  public async toggleAutomation(marketSourceId: string, isAutomationEnabled: boolean) {
    const db = Db1Connection.getInstance();
    const { marketSourceParserProfilesTable } = await import("../../infrastructure/database/postgres/schemas/db1/market-source.schema");
  ...
  public async getParserProfile(marketSourceId: string) {
    const db = Db1Connection.getInstance();
    const { marketSourceParserProfilesTable } = await import("../../infrastructure/database/postgres/schemas/db1/market-source.schema");
    const profiles = await db.select()
      .from(marketSourceParserProfilesTable)
  ...
  public async processIncomingWebhook(payload: any) {
    const db = Db1Connection.getInstance();
    ...
    if (!sourceQuery) return { success: false, reason: "No identifier provided" };
    
    const sources = await db.select().from(marketSourcesTable).where(sourceQuery).limit(1);
    if (sources.length === 0) return { success: false, reason: "No matching market source" };
    ...
    if (profile?.isAutomationEnabled && message) {
      ...
      if (extractedData?.prices?.length > 0) {
        try {
          const pricesToSubmit = extractedData.prices.map((p: any) => ({
             mandiId: mandiId,
