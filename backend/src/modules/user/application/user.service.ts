...
export class UserService {
  ...
  public async invite(input: {
    ...
    lastName: string;
    roleId: string;
    warehouseAccess: { all: boolean; warehouseIds?: string[] };
    message?: string;
    actorId: string;
  }) {
  ...
  public async listInvitations(companyId: string) {
    return this.repo.getInvitations(companyId);
  }

  public async resendInvitation(companyId: string, invitationId: string, actorId: string) {
    ...
    if (invite.status !== "pending") throw new AppError("Only pending invitations can be resent", 400, "INVALID_INVITATION_STATUS");
  ...
  public async revokeInvitation(companyId: string, invitationId: string, actorId: string) {
    const invite = await this.repo.findInvitationById(companyId, invitationId);
    if (!invite) throw new AppError("Invitation not found", 404, "INVITATION_NOT_FOUND");
    if (invite.status !== "pending") throw new AppError("Only pending invitations can be revoked", 400, "INVALID_INVITATION_STATUS");
  ...
  public async acceptInvitation(token: string, password?: string, authUserId?: string) {
    const invite = await this.repo.findInvitationByToken(token);
    if (!invite) throw new AppError("Invalid or expired invitation token", 400, "INVALID_TOKEN");
    if (invite.status !== "pending") throw new AppError("Invitation has already been accepted or revoked", 400, "INVALID_INVITATION_STATUS");
    if (invite.expiresAt < new Date()) throw new AppError("Invitation token has expired", 400, "EXPIRED_TOKEN");
  ...
  public async list(companyId: string, params?: any) {
    if (params) {
  ...
  public async exportDirectory(companyId: string, params: any): Promise<string> {
  ...
  public async get(companyId: string, userId: string) {
  ...
  public async update(companyId: string, userId: string, actorId: string, input: Record<string, unknown>) {
  ...
  public async assignRoles(companyId: string, userId: string, actorId: string, input: { roleIds: string[] }) {
    // await SubscriptionLimitService.assertCanUpdate(companyId);
  ...
  public async status(companyId: string, userId: string, actorId: string, status: string) {
    // await SubscriptionLimitService.assertCanUpdate(companyId);
    await this.get(companyId, userId);
  ...
  public async preferences(companyId: string, userId: string, actorId: string, input: Record<string, unknown>) {
    // await SubscriptionLimitService.assertCanUpdate(companyId);
    await this.get(companyId, userId);
  ...
  public async bulkAction(
    companyId: string,
    actorId: string,
    payload: { ids: string[]; action: "activate" | "deactivate" | "lock" | "unlock" | "delete" | "assign-roles"; roleIds?: string[] }
  ) {
  ...
  public async getUserSessions(companyId: string, userId: string) {
    const db = Db1Connection.getInstance();
    return db
      .select()
      .from(sessionsTable)
  ...
  public async getUserActivity(companyId: string, userId: string) {
    const db = Db1Connection.getInstance();
    return db
      .select()
      .from(auditLogsTable)
  ...
  public async getWarehouseAccess(companyId: string, userId: string) {
  ...
  public async updateWarehouseAccess(companyId: string, userId: string, actorId: string, access: { all: boolean; warehouseIds?: string[] }) {
    // await SubscriptionLimitService.assertCanUpdate(companyId);
    await this.get(companyId, userId);
  ...
  public async resetPassword(companyId: string, userId: string, actorId: string) {
    await this.get(companyId, userId);
  ...
  public async resetMfa(companyId: string, userId: string, actorId: string) {
    await this.get(companyId, userId);
    const db = Db1Connection.getInstance();
  ...
  public async terminateSessions(companyId: string, userId: string, actorId: string) {
    await this.get(companyId, userId);
  ...
  private async checkUserIsAdmin(companyId: string, userId: string): Promise<boolean> {
    const db = Db1Connection.getInstance();
    const adminRoles = await db
      ...
      .where(
        and(
          ...
          or(
            eq(rolesTable.name, "System Administrator"),
            eq(rolesTable.name, "Organization Administrator"),
            eq(rolesTable.name, "Administrator")
          ),
  ...
  private async checkUserIsSuperAdmin(companyId: string, userId: string): Promise<boolean> {
    const db = Db1Connection.getInstance();
    const adminRoles = await db
      .select({ id: userRolesTable.id })
      .from(userRolesTable)
  ...
  private async checkRolesIncludeAdmin(companyId: string, roleIds: string[]): Promise<boolean> {
