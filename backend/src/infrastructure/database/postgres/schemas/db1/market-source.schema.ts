...
import { statesTable, districtsTable, mandisTable, productsTable, productVariantsTable, usersTable } from "./all.schema";
...
export const marketSourcesTable = pgTable("market_sources", {
  ...
  email: varchar("email"),
  mandiId: uuid("mandi_id").references((): AnyPgColumn => mandisTable.id),
  ...
  userId: uuid("user_id").references((): AnyPgColumn => usersTable.id), // For future login
...
export const marketSourceProductsTable = pgTable("market_source_products", {
  id: uuid("id").primaryKey().defaultRandom(),
  marketSourceId: uuid("market_source_id").notNull().references((): AnyPgColumn => marketSourcesTable.id, { onDelete: "cascade" }),
...
export const marketSourceParserProfilesTable = pgTable("market_source_parser_profiles", {
  id: uuid("id").primaryKey().defaultRandom(),
  marketSourceId: uuid("market_source_id").notNull().unique().references((): AnyPgColumn => marketSourcesTable.id, { onDelete: "cascade" }),
...
export const marketSourceAnalyticsTable = pgTable("market_source_analytics", {
  ...
  mostReportedProductId: uuid("most_reported_product_id").references((): AnyPgColumn => productsTable.id),
