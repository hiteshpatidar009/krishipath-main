...
export class FuzzyMessageParserService extends BaseService {
    ...
    async parseRawMessage(rawText, profile) {
        ...
        products.forEach(p => {
            ...
            if (supportedCropIds && Array.isArray(supportedCropIds) && supportedCropIds.length > 0) {
                if (!supportedCropIds.includes(p.id))
                    return;
            }
        ...
        for (const line of lines) {
            ...
            if (matchedCrop) {
                ...
                this.currentCropContext = activeCrop;
            }
            else if (this.currentCropContext) {
                activeCrop = this.currentCropContext;
            }
        ...
        for (const p of parsedPrices) {
            if (!groupedByCrop[p.productId])
                groupedByCrop[p.productId] = [];
        ...
        for (const [pid, pricesList] of Object.entries(groupedByCrop)) {
            ...
            for (const p of pricesList) {
                ...
                if (p === avgVariant || p === explicitBase)
                    continue;
                finalPrices.push({
