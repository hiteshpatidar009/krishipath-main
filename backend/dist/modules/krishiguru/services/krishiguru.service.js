import { GeminiGateway } from "../gateway/ai-gateway";
...
export class KrishiGuruService {
    ...
    eventDispatcher;
    aiGateway;
    constructor(krishiGuruRepository, eventDispatcher) {
        this.krishiGuruRepository = krishiGuruRepository;
        this.eventDispatcher = eventDispatcher;
        this.aiGateway = new GeminiGateway();
    }
    async farmerId(userId, claimedFarmerId) {
    ...
    async getHistory(userId, claimedFarmerId) {
    ...
    async chat(userId, claimedFarmerId, rawMessage, language) {
        const DAILY_LIMIT = 100;
        const message = String(rawMessage || "").trim();
        if (!message)
            throw new AppError("Message is required", 400, "MESSAGE_REQUIRED");
        if (message.length > 2000)
            throw new AppError("Message is too long", 422, "MESSAGE_TOO_LONG");
        const farmerId = await this.farmerId(userId, claimedFarmerId);
