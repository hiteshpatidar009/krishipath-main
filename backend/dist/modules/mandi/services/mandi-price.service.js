...
export class MandiPriceService extends BaseService {
    ...
    traderPriceRepo;
    priceRepo;
    constructor(mandiPriceRepo, mandiAdminRepo, mandiTraderRepo, traderPriceRepo, priceRepo) {
    ...
    async getTraderPrices(mandiId, variantId, priceDate) {
        if (!mandiId || !variantId || !priceDate)
            throw new AppError("mandiId, variantId, and priceDate are required", 400);
    ...
    async updateTraderPrice(traderId, mandiId, variantId, pricePerQuintal, updatedBy, priceDate, grade) {
        if (!traderId || !mandiId || !variantId || !pricePerQuintal) {
            throw new AppError("Missing required fields for trader price", 400);
        }
        if (!updatedBy)
            throw new AppError("An authenticated user is required to publish a trader price", 401);
     ...
     */
    async updateCropPriceSheet(data) {
        if (!data.updatedBy)
            throw new AppError("An authenticated user is required to save prices", 401);
        ...
        if (traderPrices.length > 100)
            throw new AppError("A maximum of 100 trader prices is allowed", 400);
        ...
        for (const offer of traderPrices) {
            await this.updateTraderPrice(offer.traderId, data.mandiId, data.productId, offer.pricePerQuintal, data.updatedBy, data.priceDate, offer.grade);
        }
        return this.getTraderPrices(data.mandiId, data.productId, data.priceDate);
    }
    async getOfficialPrices(mandiId) {
    ...
    async setOfficialPrices(mandiId, records, setBy) {
        ...
        if (records.length > 500)
            throw new AppError("A maximum of 500 price records is allowed", 400);
     ...
     */
    async copyPrices(sourceMandiId, targetMandiId, variantIds, setBy) {
        ...
        await this.mandiPriceRepo.bulkInsert(filtered.map((p) => ({
            id: randomUUID(),
