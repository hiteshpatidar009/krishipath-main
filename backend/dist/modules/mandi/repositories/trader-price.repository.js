...
import { traderMandiPricesTable, tradersTable, mandiTraderAssignmentsTable, usersTable, } from "../../../infrastructure/database/postgres/schemas/db1/all.schema";
export class TraderPriceRepository {
    get db() {
        return Db1Connection.getInstance();
    }
    async findByMandiAndVariant(mandiId, variantId) {
        ...
        const traders = await this.db
            ...
            .leftJoin(traderMandiPricesTable, and(eq(traderMandiPricesTable.traderId, tradersTable.id), eq(traderMandiPricesTable.mandiId, mandiId), eq(traderMandiPricesTable.variantId, variantId)))
            .where(and(eq(mandiTraderAssignmentsTable.mandiId, mandiId), eq(mandiTraderAssignmentsTable.status, 'ACTIVE')));
        return traders;
    }
    /**
     ...
     */
    async findByMandiAndProduct(mandiId, productId) {
        return this.db
            .select({
            traderId: mandiTraderAssignmentsTable.traderId,
            shopName: tradersTable.shopName,
            ...
            .leftJoin(traderMandiPricesTable, and(eq(traderMandiPricesTable.traderId, tradersTable.id), eq(traderMandiPricesTable.mandiId, mandiId), eq(traderMandiPricesTable.variantId, productId)))
            .where(and(eq(mandiTraderAssignmentsTable.mandiId, mandiId), eq(mandiTraderAssignmentsTable.status, "ACTIVE"), eq(tradersTable.isActive, true), eq(tradersTable.verificationStatus, "APPROVED")));
    }
