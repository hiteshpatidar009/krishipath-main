...
import { officialMandiPricesTable, productsTable, } from "../../../infrastructure/database/postgres/schemas/db1/all.schema";
export class MandiPriceRepository {
    get db() {
        return Db1Connection.getInstance();
    }
    async findLatestByMandi(mandiId) {
        return this.db
            .select({
    ...
    async findHistoryByMandi(mandiId, days = 30) {
        const from = new Date();
        ...
        return this.db
            ...
            .from(officialMandiPricesTable)
            .innerJoin(productsTable, eq(officialMandiPricesTable.cropId, productsTable.id))
            .where(and(eq(officialMandiPricesTable.mandiId, mandiId), gte(officialMandiPricesTable.priceDate, fromDate)))
            .orderBy(desc(officialMandiPricesTable.priceDate));
    }
    /** Latest rows for a product across a set of mandis (used for comparison). */
    async findByProductAcrossMandis(productId, mandiIds) {
        ...
        return this.db
            ...
            .from(officialMandiPricesTable)
            .where(and(eq(officialMandiPricesTable.cropId, productId), inArray(officialMandiPricesTable.mandiId, mandiIds)))
            .orderBy(desc(officialMandiPricesTable.priceDate));
    }
    async findOne(mandiId, variantId, priceDate) {
        const result = await this.db
            .select()
            .from(officialMandiPricesTable)
            .where(and(eq(officialMandiPricesTable.mandiId, mandiId), eq(officialMandiPricesTable.cropId, variantId), eq(officialMandiPricesTable.priceDate, priceDate)))
            .limit(1);
