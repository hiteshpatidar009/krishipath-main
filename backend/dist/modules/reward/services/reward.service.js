...
export class RewardService {
    ...
    async redeem(userId, catalogItemId) {
        if (!catalogItemId)
            throw new AppError("Reward id is required", 400, "REWARD_ID_REQUIRED");
    ...
    async updateCatalogItem(id, data) {
        ...
        if (data.description !== undefined)
            patch.description = data.description ? String(data.description).trim() : null;
        if (data.icon !== undefined)
            patch.icon = data.icon ? String(data.icon).slice(0, 16) : null;
        if (data.isActive !== undefined)
            patch.isActive = Boolean(data.isActive);
        if (data.pointsCost !== undefined) {
            const cost = Number(data.pointsCost);
            if (!Number.isInteger(cost) || cost <= 0)
                throw new AppError("pointsCost must be positive", 422, "INVALID_REWARD_COST");
            patch.pointsCost = cost;
        }
        if (data.stock !== undefined) {
            const stock = data.stock == null ? null : Number(data.stock);
            if (stock != null && (!Number.isInteger(stock) || stock < 0))
                throw new AppError("stock is invalid", 422, "INVALID_REWARD_STOCK");
            patch.stock = stock;
        }
    ...
    async awardPoints(farmerId, actionId, points, description) {
