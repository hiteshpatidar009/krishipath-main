...
export class MarketInsightService extends BaseService {
    ...
    async createInsight(input, createdBy) {
        if (!createdBy)
            throw new AppError("An authenticated admin is required", 401);
        const patch = this.normalize(input, false);
        if (!patch.productId)
            throw new AppError("productId is required", 422, "PRODUCT_REQUIRED");
        if (!patch.scope)
            throw new AppError("scope is required", 422, "SCOPE_REQUIRED");
        if (!patch.recommendation)
            throw new AppError("recommendation is required", 422, "RECOMMENDATION_REQUIRED");
    ...
    async updateInsight(id, input) {
        const current = await this.insightRepo.findById(id);
        if (!current)
            throw new AppError("Market insight not found", 404, "INSIGHT_NOT_FOUND");
        const patch = this.normalize(input, true);
        if (!Object.keys(patch).length)
            throw new AppError("No editable insight fields provided", 400, "EMPTY_UPDATE");
        this.assertPublishable({ ...current, ...patch });
        return this.insightRepo.update(id, patch);
    }
    normalize(input, partial) {
        ...
        if (!partial) {
            ...
            if (input?.status !== undefined)
                output.status = String(input.status).toUpperCase();
            if (input?.source !== undefined)
                output.source = String(input.source).toUpperCase();
        }
        if (output.scope)
            output.scope = String(output.scope).toUpperCase();
        if (output.recommendation)
            output.recommendation = String(output.recommendation).toUpperCase();
        const numericFields = [
            ...
            if (input?.[field] !== undefined) {
                if (input[field] === null || input[field] === "")
                    ...
                    if (!Number.isFinite(value))
                        throw new AppError(`${field} must be numeric`, 422, "INVALID_NUMBER");
        ...
        for (const field of ["positiveFactors", "riskFactors"]) {
            if (input?.[field] !== undefined) {
                if (!Array.isArray(input[field]))
                    throw new AppError(`${field} must be an array`, 422, "INVALID_FACTORS");
        ...
        for (const field of ["bestWindowFrom", "bestWindowTo"]) {
            if (input?.[field] !== undefined) {
                ...
                if (value && !/^\d{4}-\d{2}-\d{2}$/.test(value))
                    throw new AppError(`${field} must use YYYY-MM-DD`, 422, "INVALID_DATE");
                output[field] = value;
            }
        }
        for (const field of ["publishAt", "expiresAt"]) {
            if (input?.[field] !== undefined) {
                ...
                if (value && Number.isNaN(value.getTime()))
                    throw new AppError(`${field} must be a valid date`, 422, "INVALID_DATE");
                output[field] = value;
            }
        }
        if (output.status === "PUBLISHED" && input?.publishAt === undefined)
            output.publishAt = new Date();
        return output;
    }
    assertPublishable(data) {
        ...
        if (data.scope && !scopes.includes(data.scope))
            throw new AppError("scope is invalid", 422, "INVALID_SCOPE");
        if (data.recommendation && !recommendations.includes(data.recommendation))
            throw new AppError("recommendation is invalid", 422, "INVALID_RECOMMENDATION");
        if (data.status && !statuses.includes(data.status))
            throw new AppError("status is invalid", 422, "INVALID_STATUS");
        if (data.source && !sources.includes(data.source))
            throw new AppError("source is invalid", 422, "INVALID_SOURCE");
        if (data.scope === "MANDI" && !data.mandiId)
            throw new AppError("mandiId is required for MANDI scope", 422, "MANDI_REQUIRED");
