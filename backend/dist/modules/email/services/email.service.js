...
export class EmailService {
    ...
    notificationLogRepo = new MongoNotificationLogRepository();
    constructor() {
    ...
    async send(dto) {
        if (!this.enabled) {
            if (env.isEnvironmentProduction) {
                throw new AppError("Brevo not configured", 503, "BREVO_NOT_CONFIGURED");
            }
            ...
            this.notificationLogRepo
                ...
                .catch((err) => logger.error(err instanceof Error ? err : new Error(String(err)), { module: "email.service", tags: ["notification_log", "mongo"] }));
            // ─────────────────────────────────────────────────────────────────────────
        ...
        const result = dto.singleAttempt ?
            await send()
        ...
        const messageId = String(result?.messageId ?? `brevo-${Date.now().toString()}`);
