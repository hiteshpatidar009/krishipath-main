...
export class SmsService {
    ...
    async send(dto) {
        ...
        if (!accountSid || !/^AC[a-fA-F0-9]{32}$/.test(accountSid) || !authToken || !configuredFrom) {
            throw new AppError("SMS delivery is not configured. Set valid Twilio credentials on the backend.", 503, "SMS_PROVIDER_NOT_CONFIGURED");
        }
        ...
        try {
            ...
            this.notificationLogRepo.create({
                ...
                companyId: dto.companyId,
                userId: dto.userId,
            }).catch((error) => logger.error(error instanceof Error ? error : new Error(String(error)), { module: "sms.service", tags: ["notification_log", "mongo"] }));
