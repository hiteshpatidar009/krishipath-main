...
export class IdempotencyService {
    ...
    async get(companyId, key) {
      ...
      LIMIT 1
    `);
        const row = SqlResult.rows(result)[0];
        if (!row) {
            return null;
        }
    ...
    async reserveExecution(companyId, key, requestHash, ttlMinutes = 60) {
    ...
    async reserve(companyId, key, requestHash, ttlMinutes = 60) {
    ...
    async complete(companyId, key, requestHash, statusCode, responseSnapshot) {
        await Db1Connection.getInstance().execute(sql `
    ...
    async fail(companyId, key, requestHash) {
        await Db1Connection.getInstance().execute(sql `
    ...
    async retryFailed(companyId, key, requestHash, _ttlMinutes = 60) {
        const result = await Db1Connection.getInstance().execute(sql `
      DELETE FROM idempotency_keys
      WHERE idempotency_key = ${key}
        AND request_hash = ${requestHash}
