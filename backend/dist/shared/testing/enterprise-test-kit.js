...
export class EnterpriseTestMetrics {
    ...
    markdown(moduleName) {
        ...
        const passed = this.records.filter((record) => record.passFailStatus === "pass").length;
        const failed = this.records.filter((record) => record.passFailStatus === "fail").length;
        const warnings = this.records.filter((record) => record.passFailStatus === "warning").length;
        return [
    ...
    apiMarkdown(moduleName) {
        return tableMarkdown(`${moduleName} API Tests`, this.records.filter((record) => record.endpoint));
    }
    categoryMarkdown(moduleName, title, category) {
        return tableMarkdown(`${moduleName} ${title}`, this.records.filter((record) => record.testCategory === category));
    }
    functionMarkdown(moduleName) {
        return [
            `# ${moduleName} Function Inventory Tests`,
            "",
            "| Kind | Name | File |",
            "|---|---|---|",
            ...this.functions.map((record) => `| ${record.kind} | ${record.name} | ${record.file} |`),
            "",
        ].join("\n");
    }
    jsonSummary(moduleName) {
        ...
        return {
            ...
            counts: {
                ...
                fail: this.records.filter((record) => record.passFailStatus === "fail")
                    .length,
                warning: this.records.filter((record) => record.passFailStatus === "warning").length,
            },
            latency,
...
export function readModuleSource(moduleDir) {
    return scanModule(moduleDir)
        .filter((file) => file.endsWith(".ts"))
        .map((file) => readFileSync(file, "utf8"))
        .join("\n");
}
export function discoverEndpoints(moduleDir) {
    const records = [];
    for (const file of scanModule(moduleDir).filter((item) => item.includes("routes") && item.endsWith(".ts"))) {
        const content = readFileSync(file, "utf8");
        const pattern = /this\.router\.(get|post|put|patch|delete)\(\s*["']([^"']+)["']/g;
        let match;
...
export function discoverFunctions(moduleDir) {
    const records = [];
    for (const file of scanModule(moduleDir).filter((item) => item.endsWith(".ts"))) {
        const content = readFileSync(file, "utf8");
        const relativeFile = relative(moduleDir, file);
        collectMatches(content, /export\s+class\s+([A-Za-z0-9_]+)/g, (name) => records.push({ kind: "class", name, file: relativeFile }));
        collectMatches(content, /export\s+(?:async\s+)?function\s+([A-Za-z0-9_]+)\s*\(/g, (name) => records.push({ kind: "function", name, file: relativeFile }));
        collectMatches(content, /\b(?:public|private|protected)\s+(?:async\s+)?([A-Za-z_][A-Za-z0-9_]*)\s*[=(]/g, (name) => records.push({ kind: "method", name, file: relativeFile }));
    }
    return records;
}
export async function measure(metrics, name, runner) {
...
export async function runConcurrent(count, runner) {
    return Promise.all(Array.from({ length: count }, (_, index) => runner(index)));
}
export function writeModuleReports(moduleName, metrics) {
    ...
    const details = {
        ...
        "security-tests.md": metrics.categoryMarkdown(moduleName, "Security Tests", "security"),
        "validation-tests.md": metrics.categoryMarkdown(moduleName, "Validation Tests", "validation"),
        "middleware-tests.md": metrics.categoryMarkdown(moduleName, "Middleware Tests", "middleware"),
        "performance-tests.md": JSON.stringify(metrics.summary(), null, 2),
        "concurrency-tests.md": metrics.categoryMarkdown(moduleName, "Concurrency Tests", "concurrency"),
        "workflow-tests.md": metrics.categoryMarkdown(moduleName, "Workflow Tests", "workflow"),
        "event-tests.md": metrics.categoryMarkdown(moduleName, "Event Tests", "event"),
        "transaction-tests.md": metrics.categoryMarkdown(moduleName, "Transaction Tests", "transaction"),
        "observability-tests.md": metrics.categoryMarkdown(moduleName, "Observability Tests", "observability"),
        "failures.md": failureMarkdown(moduleName, metrics.allRecords()),
        "recommendations.md": recommendationMarkdown(moduleName, metrics.allRecords()),
...
export function registerEnterpriseModuleTests(config) {
    ...
    describe(`${config.moduleName} enterprise module`, () => {
        ...
        it("has required modular boundary files", async () => {
            await recordTest(metrics, config.moduleName, "architecture", "module boundary files", undefined, "module.ts and index.ts exist", async () => {
        ...
        it("has no empty placeholder exports", async () => {
            await recordTest(metrics, config.moduleName, "architecture", "placeholder export scan", undefined, "no export {}; placeholders", async () => {
                const offenders = moduleFiles().filter((file) => readFileSync(file, "utf8").trim() === "export {};");
                expect(offenders.map((file) => relative(config.moduleDir, file))).toEqual([]);
                return "no-placeholders";
            });
        });
        it("keeps enterprise layer artifacts discoverable", async () => {
            await recordTest(metrics, config.moduleName, "architecture", "layer artifact scan", undefined, "routes/controllers/services/domain discoverable", async () => {
                const files = moduleFiles();
                expect(files.some((file) => file.includes("routes") || file.includes("presentation"))).toBe(true);
                expect(files.some((file) => file.includes("controller") || file.includes("use-cases"))).toBe(true);
                expect(files.some((file) => file.includes("service") || file.includes("domain"))).toBe(true);
                return "layers-discovered";
            });
        });
        it("enforces route authentication and authorization where required", async () => {
            await recordTest(metrics, config.moduleName, "security", "route guard source scan", undefined, "auth/authorization guards present", async () => {
                const routeFiles = moduleFiles().filter((file) => file.includes("routes") && file.endsWith(".ts"));
                ...
                if (config.requiresAuth !== false) {
                    ...
                    expect(routeSource).toMatch(/AuthorizationMiddleware|ensureFullAccess|CompanyGuard/);
                }
                const routeCount = (routeSource.match(/router\.(get|post|put|patch|delete)/g) ?? []).length;
                expect(routeCount).toBeGreaterThanOrEqual(config.expectedRouteCount ?? 1);
        ...
        it("contains validation or controlled DTO parsing readiness", async () => {
            await recordTest(metrics, config.moduleName, "validation", "validation source scan", undefined, "validators or parsers present", async () => {
        ...
        it("contains observability hooks", async () => {
            await recordTest(metrics, config.moduleName, "observability", "observability source scan", undefined, "logging/events/context responses present", async () => {
                expect(source()).toMatch(/logger\.(info|warn|error|debug)|audit|Activity|Trace|requestId|CoreEventBus|EventEnvelopeFactory|RequestContext|ApiResponse/);
                return "observability-ready";
            });
        });
        it("contains event/workflow readiness", async () => {
            await recordTest(metrics, config.moduleName, "event", "event and workflow readiness scan", undefined, "events or explicit no-op flows located", async () => {
                const text = source();
                const hasEventOrWorkflow = /Events|CoreEventBus|workflow|approval|orchestration|contract/i.test(text);
                expect(hasEventOrWorkflow).toBe(true);
                return "event-workflow-ready";
            });
        });
        it("contains transaction or rollback readiness", async () => {
            await recordTest(metrics, config.moduleName, "transaction", "transaction and rollback readiness scan", undefined, "transaction/rollback/idempotency/repository boundary present", async () => {
                const text = source();
                if (!/transaction|rollback|idempot|repository|Repository|create\(|update\(/i.test(text)) {
                    metrics.finding("transaction/idempotency marker missing; add explicit retry/idempotency adapter when module mutates external state");
        ...
        it("supports module construction without database side effects", async () => {
            await recordTest(metrics, config.moduleName, "unit", "module construction", undefined, "module constructs and router exists", async () => {
        ...
        for (const record of functions) {
            it(`${record.kind} ${record.name} source-level unit coverage`, async () => {
                await recordTest(metrics, config.moduleName, "unit", `${record.kind} ${record.name}`, undefined, "function source exists and is inventoried", async () => {
                    metrics.functionRecord(record);
                    const fileContent = readFileSync(join(config.moduleDir, record.file), "utf8");
        ...
        it("handles concurrent metadata scans deterministically", async () => {
            await recordTest(metrics, config.moduleName, "concurrency", "parallel metadata scan", undefined, "all concurrent scans return same count", async () => {
        ...
        it("emits per-module enterprise report files", async () => {
            await recordTest(metrics, config.moduleName, "observability", "report generation", undefined, "all report files generated", async () => {
                writeModuleReports(config.moduleName, metrics);
                for (const fileName of reportFiles) {
                    expect(existsSync(join(process.cwd(), "reports", config.moduleName, fileName))).toBe(true);
                }
...
function endpointScenarios(endpoint) {
    const scenarios = [
        {
    ...
    return scenarios.filter((scenario) => endpoint.method !== "GET" ||
        scenario.name === "blocks unauthenticated access");
}
async function recordEndpointTest(metrics, config, endpoint, scenario) {
    ...
    try {
        ...
        const module = new config.ModuleClass();
        app.use(module.getRouter?.());
        const response = await sendRequest(app, endpoint, scenario);
        status = response.status;
        body = response.body;
        const publicStatus = isPublicSuccessEndpoint(endpoint.path) && response.status < 500;
        ...
        metrics.endpoint({
            ...
            result: actual,
            error: protectedBlocked || publicStatus ? undefined : JSON.stringify(body),
        });
        ...
        metrics.record(`${endpoint.method} ${endpoint.path} ${scenario.name}`, duration);
        metrics.testCase({
            ...
            securityNotes: scenario.category === "security" ?
                "malicious payload sent through route stack"
                : undefined,
            validationNotes: scenario.category === "validation" ?
                "malformed payload sent through route stack"
                : undefined,
        });
    }
}
async function sendRequest(app, endpoint, scenario) {
    const path = routePathForRequest(endpoint.path);
    const client = request(app);
    let agent = endpoint.method === "POST" ? client.post(path)
    ...
    if (endpoint.method !== "GET" && endpoint.method !== "DELETE") {
        agent = agent.send(scenario.payload);
    }
    return agent;
}
async function recordTest(metrics, moduleName, category, scenario, payload, expected, runner, notes = {}) {
    const started = performance.now();
    let actual = "not-run";
    try {
        actual = await runner();
...
function collectMatches(content, pattern, onMatch) {
    let match;
    while ((match = pattern.exec(content)) !== null) {
        onMatch(match[1]);
    }
...
function routePathForRequest(routePath) {
    return routePath.replace(/:[A-Za-z0-9_]+/g, "11111111-1111-4111-8111-111111111111");
}
function isPublicSuccessEndpoint(routePath) {
...
function tableMarkdown(title, records) {
    return [
        `# ${title}`,
        "",
        "| Category | Scenario | Method | Endpoint | Expected | Actual | Status | Response Time | Result | Errors |",
        "|---|---|---|---|---|---|---:|---:|---|---|",
        ...records.map((record) => `| ${record.testCategory} | ${record.testScenario} | ${record.method ?? ""} | ${record.endpoint ?? ""} | ${record.expectedResult} | ${record.actualResult} | ${record.responseStatus ?? ""} | ${record.responseTimeMs.toFixed(3)}ms | ${record.passFailStatus} | ${(record.errors ?? []).join(", ")} |`),
        "",
    ].join("\n");
}
function failureMarkdown(moduleName, records) {
...
function recommendationMarkdown(moduleName, records) {
    const warnings = records.filter((record) => record.passFailStatus === "warning");
    const failures = records.filter((record) => record.passFailStatus === "fail");
    ...
    return [
        `# ${moduleName} Recommendations`,
        "",
        ...failures.map((record) => `- fix failed scenario: ${record.testScenario}`),
        ...(warnings.length ?
            warnings.map((record) => `- warning: ${record.testScenario} returned ${record.actualResult}`)
            : []),
        "",
    ].join("\n");
}
function coverageJson(moduleName, metrics) {
    const records = metrics.allRecords();
    const count = (category) => records.filter((record) => record.testCategory === category).length;
    return {
        moduleName,
        routeCoverage: count("middleware") + count("security") + count("validation"),
