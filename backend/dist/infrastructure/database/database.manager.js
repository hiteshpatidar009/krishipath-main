...
class DbConnAdapter {
    ...
    onConnect;
    onDisconnect;
    constructor(onConnect, onDisconnect, init) {
        ...
        return [
            {
                ...
                optional: true,
                connection: new DbConnAdapter(() => MongoDbConnection.connect(), () => MongoDbConnection.disconnect()),
            },
            {
                name: "redis",
                connection: new DbConnAdapter(() => RedisConnection.connect(), () => RedisConnection.disconnect(), () => {
                    RedisConnection.getInstance();
                }),
            },
            {
                name: "db1",
                connection: new DbConnAdapter(() => Db1Connection.connect(), () => Db1Connection.disconnect(), () => {
...
export class DatabaseManager {
    ...
    async connectAll() {
        ...
        try {
            for (const managedConnection of this.connections) {
                try {
                    const connectionLabel = DATABASE_CONNECTION_NAMES[managedConnection.name];
                    ...
                    const connectionLabel = DATABASE_CONNECTION_NAMES[managedConnection.name];
                    const normalizedError = error instanceof Error ? error : (new Error(`Failed to connect ${connectionLabel}`));
                    ...
                    if (managedConnection.optional) {
                        ConsoleTagLogger.warn("DATABASE", `${connectionLabel} is optional; continuing without it`);
                    }
                    else {
    ...
    async disconnectAll() {
        ...
        const disconnectResults = await Promise.allSettled(this.connections.map((managedConnection) => managedConnection.connection.disconnect()));
        for (let index = 0; index < disconnectResults.length; index++) {
            ...
            if (result.status === "rejected") {
                const connectionLabel = DATABASE_CONNECTION_NAMES[managedConnection.name];
                const normalizedError = result.reason instanceof Error ?
                    result.reason
    ...
    async rollbackConnectedDatabases(connectedConnections) {
        const rollbackConnections = [...connectedConnections].reverse();
        const rollbackResults = await Promise.allSettled(rollbackConnections.map((managedConnection) => managedConnection.connection.disconnect()));
        for (let index = 0; index < rollbackResults.length; index++) {
            ...
            if (rollbackResult.status === "rejected") {
                const connectionLabel = DATABASE_CONNECTION_NAMES[managedConnection.name];
                const normalizedError = rollbackResult.reason instanceof Error ?
