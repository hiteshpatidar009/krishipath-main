...
const SQL_TABLE_PATTERN = /CREATE TABLE IF NOT EXISTS\s+"?([a-zA-Z0-9_]+)"?\s*\(([\s\S]*?)\n\);/g;
const DRIZZLE_TABLE_PATTERN = /pgTable\(\s*"([a-zA-Z0-9_]+)"\s*,\s*\{([\s\S]*?)\n\s*\}\s*,?\s*\)/g;
const DRIZZLE_COLUMN_PATTERN = /[a-zA-Z0-9_]+\s*:\s*[a-zA-Z0-9_]+\(\s*"([a-zA-Z0-9_]+)"/g;
...
class SchemaDriftCheck {
    ...
    compare(targetName, sqlTables, drizzleTables) {
        let driftCount = 0;
        const sqlOnlyTables = [...sqlTables.keys()].filter((tableName) => !drizzleTables.has(tableName));
        const drizzleOnlyTables = [...drizzleTables.keys()].filter((tableName) => !sqlTables.has(tableName));
        ...
        for (const [tableName, sqlTable] of sqlTables.entries()) {
            ...
            const sqlOnlyColumns = [...sqlTable.columns].filter((column) => !drizzleTable.columns.has(column));
            const drizzleOnlyColumns = [...drizzleTable.columns].filter((column) => !sqlTable.columns.has(column));
            if (sqlOnlyColumns.length === 0 && drizzleOnlyColumns.length === 0) {
                continue;
            }
            driftCount += 1;
            this.write(`  ${tableName}`);
            this.write(`    SQL-only columns: ${sqlOnlyColumns.join(", ") || "none"}`);
            this.write(`    Drizzle-only columns: ${drizzleOnlyColumns.join(", ") || "none"}`);
        }
        if (driftCount === 0) {
