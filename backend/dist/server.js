...
export class Server {
    ...
    async start() {
        try {
            ConsoleTagLogger.info("SERVER", `Starting server on port ${this.port}...`);
            ...
            await new Promise((resolve, reject) => {
                ...
                this.httpServer?.listen(this.port, () => {
                    ...
                    ConsoleTagLogger.info("SERVER", `Server is running on http://${resolvedAddress.address}:${resolvedAddress.port}`);
                    void logger.info(`Server is running on port ${resolvedAddress.port.toString()}`, {
                        module: "server",
            ...
            const normalizedError = error instanceof Error ? error : (new Error("Unknown server startup error"));
            ConsoleTagLogger.error("SERVER", `Server startup failed: ${normalizedError.message}`);
    ...
    registerShutdownHandlers() {
        ...
        const shutdown = async (signal) => {
            ConsoleTagLogger.info("SERVER", `${signal} received. Starting graceful shutdown...`);
            ...
            try {
                ...
                const shutdownError = error instanceof Error ? error : (new Error("Graceful shutdown failed"));
                ConsoleTagLogger.error("SERVER", "Graceful shutdown failed:", shutdownError.message);
    ...
    async stop() {
        ...
        await new Promise((resolve, reject) => {
            this.httpServer?.close((error) => {
                if (error) {
                    ConsoleTagLogger.error("SERVER", "HTTP server close failed:", error.message);
